...
class MergeWithClustering:
    ...
    def __init__(self):
        '''初始化'''
        self.label_dict = self.load_label_data()
        self.phish_label_set = self.load_phishing_label()
        self.life_dict = self.load_life_data()
    ...
    def draw_figures(self, save_path, comm_size_line, fitness_best_line, fitness_avg_line):
        '''绘制图表
        绘制社区大小和适应度变化的图表
    ...
    def save_isolated_comm_files(self, save_path, seed, seed_life):
        '''保存孤立社区文件
        保存只有一个节点的社区文件
    ...
    def save_comm_files(self, save_path, C, G, nodes_attr, edges_attr, idx2add):
        '''保存社区文件
        保存社区的节点和边文件
        ...
        for addr in nodes_list:
            if addr in self.label_dict: label_list.append(self.label_dict[addr])
            elif addr in self.phish_label_set: label_list.append("phish-hack")
        ...
        for addr in nodes_list_inner:
            if addr in self.label_dict: label_list_inner.append(self.label_dict[addr])
            elif addr in self.phish_label_set: label_list_inner.append("phish-hack")
